...
export function coherenceRules(doc: OcfDoc, _ctx: DocContext): Issue[] {
  ...
  frames.forEach((frame, fi) => {
    ...
    // A frame's actions execute in sequence, so when a player has several move
    // actions in one frame (e.g. dribble to X, then cut to Y, expressed with
    // `after`), only the LAST move with an explicit endpoint determines the
    // player's final position — that is what end_state must agree with. Checking
    // every action independently would flag the legitimate intermediate endpoints
    // as disagreements. We therefore compare end_state against each player's last
    // explicit-endpoint move only.
    const lastEndpoint = new Map<string, unknown>();
    for (const action of actions) {
      ...
      if (lastTo !== undefined) lastEndpoint.set(player, lastTo);
    }
