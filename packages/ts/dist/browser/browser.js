...
// ../../shared/schema/ocf-action-v1.json
...
var error_codes_default = {
  SCHEMA_INVALID: { severity: "error", category: "schema", message: "Schema validation failed: {detail}", spec_ref: "schema/v1.json" },
  MODEL_LEGACY: { severity: "error", category: "schema", message: "Document uses the superseded geometric model (entity_states/lines). Use the action model (actions/balls/end_state).", spec_ref: "design \xA71" },
  JSON_PARSE: { severity: "error", category: "schema", message: "File is not valid JSON: {detail}", spec_ref: "design \xA75" },
  ...
  BALL_AMBIGUOUS: { severity: "error", category: "ball-possession", message: "Action by '{player}' omits ball_id but {count} balls are in play.", spec_ref: "design \xA74-B" },
  ACTION_UNUSUAL_CARRIER: { severity: "warning", category: "ball-possession", message: "Defense player '{player}' performs {action}; unusual but allowed.", spec_ref: "design \xA74-B" },
  END_STATE_DISAGREE: { severity: "error", category: "coherence", message: "end_state for '{ref}' disagrees with action endpoint.", spec_ref: "design \xA74-C" },
...
// src/codes.ts
var CODES = error_codes_default;
function fill(template, data) {
  return template.replace(/\{(\w+)\}/g, (_, k) => {
    const v = data[k];
    return v !== null && v !== void 0 ? String(v) : `{${k}}`;
  });
}
function makeIssue(code, path, data = {}, frame) {
  const def = CODES[code];
  if (!def) throw new Error(`Unknown error code: ${code}`);
  return {
    code,
    severity: def.severity,
    message: fill(def.message, data),
    path,
    ...frame !== void 0 ? { frame } : {},
...
// src/schema-level.ts
var ajv = new import_ajv.default({ allErrors: true, strictSchema: false, strictRequired: false });
(0, import_ajv_formats.default)(ajv);
var validateSchema = ajv.compile(ocf_action_v1_default);
function hasLegacyShape(doc) {
  const frames = doc.frames;
  if (!Array.isArray(frames)) return false;
  return frames.some((f) => f !== null && typeof f === "object" && "entity_states" in f);
}
function schemaLevel(doc) {
  if (hasLegacyShape(doc)) return [makeIssue("MODEL_LEGACY", "/frames", {})];
  if (validateSchema(doc)) return [];
  return (validateSchema.errors ?? []).map((e) => makeIssue("SCHEMA_INVALID", e.instancePath || "/", {
...
function possessionByFrame(doc) {
  ...
  for (const frame of frames) {
    ...
    const endBalls = (frame.end_state ?? {}).balls;
    if (endBalls) {
...
// ../../shared/named-positions.json
var named_positions_default = {
  description: "Canonical OCF named court positions, resolved per ruleset at render time. Mirrored from specification-v1.adoc position tables. Custom positions are declared per-document under named_positions.custom.",
  positions: [
    "basket",
    "free_throw_line",
    "paint_center",
...
// src/named-positions.ts
var CANONICAL_NAMED = new Set(
  named_positions_default.positions
);
function knownNamed(doc) {
  const set = new Set(CANONICAL_NAMED);
  const np = doc.named_positions ?? {};
  const custom = np.custom ?? {};
...
function possessionRules(doc, ctx, states) {
  const issues = [];
  const frames = getFrames(doc);
  frames.forEach((frame, fi) => {
    const frameId = frame.id;
      ...
      if (BALL_DEPENDENT.has(type)) {
        ...
        if (ball && carrier.get(ball) !== player) {
          issues.push(makeIssue(
            ...
            path,
            { player, action: type, ball_id: ball },
            frameId
          ));
        }
...
function applyEffect(type, player, action, ball, carrier, loose) {
  switch (type) {
    case "pass": {
...
function sameNamespace(a, b) {
  return a.startsWith("named:") && b.startsWith("named:") || a.startsWith("xy:") && b.startsWith("xy:");
}
function coherenceRules(doc, _ctx) {
  const issues = [];
  const frames = getFrames(doc);
  frames.forEach((frame, fi) => {
    const frameId = frame.id;
    const endState = frame.end_state ?? {};
    const actions = frame.actions ?? [];
    const lastEndpoint = /* @__PURE__ */ new Map();
    for (const action of actions) {
      ...
      if (lastTo !== void 0) lastEndpoint.set(player, lastTo);
    }
    for (const [player, lastTo] of lastEndpoint) {
      if (!(player in endState)) continue;
      const endKey = coordKey(endState[player]);
      const toKey = coordKey(lastTo);
      if (endKey && toKey && sameNamespace(endKey, toKey) && endKey !== toKey) {
        issues.push(makeIssue(
          "END_STATE_DISAGREE",
          `/frames/${fi}/end_state/${player}`,
          { ref: player },
          frameId
        ));
      }
...
function* coords(node) {
  if (Array.isArray(node)) {
    for (const v of node) yield* coords(v);
  } else if (node && typeof node === "object") {
    const o = node;
...
function qualityRules(doc, ctx) {
  ...
  frames.forEach((frame, fi) => {
    ...
    if (actions.length === 0 && Object.keys(endState).length === 0) {
      issues.push(makeIssue("EMPTY_FRAME", `/frames/${fi}`, {}, frame.id));
    }
  });
  ...
  if (cs) {
    ...
    for (const [fillRole, strokeRole] of pairs) {
      ...
      const ratio = contrast(fill2, stroke);
